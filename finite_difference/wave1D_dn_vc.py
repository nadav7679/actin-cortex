#!/usr/bin/env python
"""
Copied from: https://github.com/hplgit/num-methods-for-PDEs/blob/master/src/wave/wave1D/wave1D_dn_vc.py
...
def test_quadratic():
    ...
    at the end x=L/2.
    """
    def u_exact(x, t): return x*(L-x)*(1+0.5*t)
    def I(x): return u_exact(x, 0)
    def V(x): return 0.5*u_exact(x, 0)
    def f(x, t): return 2*(1+0.5*t)*c**2
    def U_0(t): return u_exact(0, t)
    U_L = None
    L = 2.5
    c = 1.5
    C = 0.75
...
def test_plug():
    ...
    c = 0.5
    dt = (L/10)/c  # Nx=10
    def I(x): return 0 if abs(x-L/2.0) > 0.1 else 1
